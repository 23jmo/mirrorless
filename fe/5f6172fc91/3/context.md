...
If you're the app developer, register the JavaScript origin in the Google Cloud Console.
Request details: access_type=offline origin=https://mirrorless.vercel.app scope=openid https://www.googleapis.com/auth/userinfo.email https://www.googleapis.com/auth/userinfo.profile https://www.googleapis.com/auth/gmail.readonly https://www.googleapis.com/auth/calendar.readonly response_type=code enable_serial_consent=t...

### Prompt 24

r u sure?? - search the web

### Prompt 25

[Image: source: REDACTED 2026-02-15 07.38.41.png]
...
Now google says "load failed" on my phone

### Prompt 29

Running on logo hoes but it's f*cking working! Let's go!

### Prompt 30

Wait on the phone? The sign in with Google always says "load failed" but on the computer it works. Why is that? Using mirrorless.vercel.app

### Prompt 31

its not Wait it's not oops but is it possible for the cell, just the cue part, to all be done on the front end so that it's deployed on Vercel because I'm having a hard time deploying on render right now? The AI agent stuff works because I'm running the Miro on my personal computer so it can do localhost but yeah, can I just, I don't know, is there some way to just do the Google route through Vercel or no, like not touch the backend and not have to
